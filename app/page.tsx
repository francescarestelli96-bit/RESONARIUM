...
const CONTENT = {
  it: {
    ...
    homeTitle: '',
    homeParagraphs: [
      'Resonarium è un’associazione culturale con sede a Milano, attiva nell’ambito della musica, della formazione e della ricerca interdisciplinare. La sua attività è orientata alla costruzione di contesti di studio, ascolto e produzione culturale, con particolare attenzione al rapporto tra practice artistica, riflessione teorica e trasmissione dei saperi.',
...
function NavbarLogoDesktop() {
  return (
    <div className="relative h-24 w-64 lg:h-28 lg:w-80 transition-all">
      <Image
        src={LOGO_SRC}
        alt="Resonarium"
        fill
...
function NavbarLogoMobile() {
  return (
    <div className="mobile-logo-box relative h-16 w-48">
      <Image
        src={LOGO_SRC}
        alt="Resonarium"
        fill
...
function ContactForm({ language }: { language: Language }) {
  ...
  const setField = (field: 'name' | 'email' | 'message') => (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-10">
      <div className="grid gap-5">
        <div>
          <label htmlFor="contact-name" className="mb-2 block text-sm text-[var(--foreground-soft)]">{text.formName}</label>
          <input id="contact-name" className="form-field form-input" value={values.name} onChange={setField('name')} placeholder={text.formName} autoComplete="name" />
          {errors.name && <p className="mt-2 text-sm text-[var(--danger)]">{errors.name}</p>}
        </div>
        <div>
          <label htmlFor="contact-email" className="mb-2 block text-sm text-[var(--foreground-soft)]">{text.formEmail}</label>
          <input id="contact-email" type="email" className="form-field form-input" value={values.email} onChange={setField('email')} placeholder={text.formEmail} autoComplete="email" />
          {errors.email && <p className="mt-2 text-sm text-[var(--danger)]">{errors.email}</p>}
        </div>
        <div>
          <label htmlFor="contact-message" className="mb-2 block text-sm text-[var(--foreground-soft)]">{text.formMessage}</label>
          <textarea id="contact-message" className="form-field form-textarea" value={values.message} onChange={setField('message')} placeholder={text.formMessage} />
          {errors.message && <p className="mt-2 text-sm text-[var(--danger)]">{errors.message}</p>}
        </div>
        <div className="flex flex-col items-start gap-4 pt-2">
          <button type="submit" className="primary-button">{text.formSend}</button>
          {success && <p className="max-w-2xl text-sm leading-7 text-[var(--foreground-soft)]">{text.formSuccess}</p>}
        </div>
      </div>
    </form>
  );
...
function HomeSection({ language, onNavigate }: { language: Language; onNavigate: (section: Exclude<SectionKey, 'home'>) => void }) {
  const text = CONTENT[language];
  return (
    <section className="page-shell">
      <div className="page-content">
        ...
        <div className="body-copy mt-8 max-w-4xl md:mt-10">
          {text.homeParagraphs.map((paragraph: string) => <p key={paragraph}>{paragraph}</p>)}
        </div>
        <div className="mt-8 flex flex-wrap gap-3 md:mt-10 md:gap-4">
          <button onClick={() => onNavigate('chi-siamo')} className="primary-button">{language === 'it' ? 'Approfondisci' : 'Explore'}</button>
          <button onClick={() => onNavigate('contatti')} className="secondary-button">{language === 'it' ? 'Contatti' : 'Contacts'}</button>
        </div>
      </div>
    </section>
  );
    ...
    <section className="page-shell">
      <div className="page-content">
        ...
        <div className="body-copy mt-8 max-w-5xl md:mt-10">
          {text.aboutParagraphs.map((paragraph: string) => <p key={paragraph}>{paragraph}</p>)}
        </div>
        <div className="mt-12 md:mt-14">
          <p className="kicker">{text.foundersTitle}</p>
          <div className="mt-8 space-y-10">
            {text.founders.map((founder) => <FounderCard key={founder.name} {...founder} imageMissingLabel={text.imageMissing} />)}
          </div>
        </div>
      </div>
    </section>
  ...
  return (
    <section className="page-shell">
      <div className="page-content">
        ...
        <div className="body-copy mt-8 max-w-5xl md:mt-10">
          {text.visionParagraphs.map((paragraph: string) => <p key={paragraph}>{paragraph}</p>)}
        </div>
      </div>
    </section>
  );
        ...
        <div className="mt-10 space-y-10 md:mt-12">
          {text.activitiesItems.map((item, index) => (
            <section key={item.title} className="border-t border-[var(--line)] pt-8">
              ...
              <div className="body-copy mt-6 max-w-5xl"><p>{item.text}</p></div>
            </section>
          ))}
        </div>
      </div>
  ...
  return (
    <section className="page-shell">
      <div className="page-content">
        ...
        <div className="body-copy mt-8 max-w-5xl md:mt-10">
          {text.contactsParagraphs.map((paragraph: string) => <p key={paragraph}>{paragraph}</p>)}
        </div>
        <div className="mt-10 grid gap-10 md:mt-12 md:grid-cols-2">
          <div>
            ...
            <div className="mt-5 space-y-2 text-lg leading-9 text-[var(--foreground-soft)]">
              {text.addressLines.map((line: string) => <p key={line}>{line}</p>)}
            </div>
          </div>
          <div>
            <p className="kicker">{text.emailLabel}</p>
            <div className="mt-5 space-y-3 text-lg leading-9 text-[var(--foreground-soft)]">
              <p><a href="mailto:info@resonarium.com" className="underline underline-offset-4">{text.emailGeneral}</a></p>
              <p><a href="mailto:progetti@resonarium.com" className="underline underline-offset-4">{text.emailProjects}</a></p>
            </div>
          </div>
        </div>
...
export default function Page() {
  ...
  const closeSplash = () => {
    ...
    window.setTimeout(() => setSplashVisible(false), 560);
  };

  const renderSection = () => {
    switch (activeSection) {
      case 'home': return <HomeSection language={language} onNavigate={navigateTo} />;
      case 'chi-siamo': return <AboutSection language={language} />;
      case 'vision': return <VisionSection language={language} />;
      case 'attivita': return <ActivitiesSection language={language} />;
      case 'contatti': return <ContactsSection language={language} />;
      default: return null;
    }
  };
    ...
    <main className="site-shell frame-lines min-h-screen">
      {splashVisible && (
        <div className={cn('splash-screen', splashClosing && 'splash-fade-out')}>
          <button type="button" onClick={closeSplash} className="splash-button" aria-label={language === 'it' ? 'Entra nel sito' : 'Enter the site'}>
            <div className="splash-shell mobile-splash-shell">
              <SplashLogo />
              <h1 className="mobile-splash-title mt-10 text-[clamp(2rem,4vw,3.6rem)] tracking-[0.08em] [font-variant-caps:all-small-caps] text-[var(--foreground)]">Resonarium</h1>
              <p className="mobile-splash-subtitle mt-5 text-[0.8rem] uppercase tracking-[0.32em] text-[var(--foreground-faint)]">{text.splashSubtitle}</p>
      ...
      {!splashVisible && (
        <>
          <header className="topbar">
            <div className="container-shell">
              <div className="hidden min-h-[104px] items-center justify-between gap-4 sm:min-h-[112px] md:flex">
                <button onClick={() => navigateTo('home')} className="shrink-0 flex items-center" aria-label={language === 'it' ? 'Vai alla home' : 'Go to home'}>
                  <NavbarLogoDesktop />
                </button>
                <div className="hidden min-w-0 flex-1 px-3 xl:block">
                  <nav className="flex items-center justify-center gap-6">
                    <button onClick={() => navigateTo('home')} className={cn('nav-link', activeSection === 'home' && 'active')}>{text.navHome}</button>
                    {NAV_ITEMS.map((item) => (
                      <button key={item.key} onClick={() => navigateTo(item.key)} className={cn('nav-link', activeSection === item.key && 'active')}>{item.label[language]}</button>
                    ))}
                  </nav>
                </div>
                <div className="hidden shrink-0 items-center gap-3 md:flex">
                  <button onClick={() => setLanguage('it')} className={cn('secondary-button', language === 'it' && 'active-lang')}>ITA</button>
                  <button onClick={() => setLanguage('en')} className={cn('secondary-button', language === 'en' && 'active-lang')}>ENG</button>
                </div>
              </div>

              <div className="mobile-header-row flex items-center justify-between md:hidden">
                <button onClick={() => navigateTo('home')} className="flex items-center" aria-label={language === 'it' ? 'Vai alla home' : 'Go to home'}>
                  <NavbarLogoMobile />
                </button>
                <div className="mobile-actions">
                  <button onClick={() => setLanguage('it')} className={cn('secondary-button', language === 'it' && 'active-lang')}>ITA</button>
                  <button onClick={() => setLanguage('en')} className={cn('secondary-button', language === 'en' && 'active-lang')}>ENG</button>
                  <button onClick={() => setMobileMenuOpen(true)} className="secondary-button menu-button" aria-label={text.menu}>{text.menu}</button>
                </div>
              </div>
            </div>
          </header>
            ...
            <div className="mobile-menu-overlay menu-fade-in md:hidden">
              <div className="container-shell mobile-menu-panel flex min-h-screen flex-col py-8">
                <div className="flex items-center justify-between">
                  <button onClick={() => navigateTo('home')} className="flex items-center"><NavbarLogoMobile /></button>
                  <button onClick={() => setMobileMenuOpen(false)} className="secondary-button">{text.close}</button>
                </div>
                <div className="mt-12 flex flex-1 flex-col justify-center gap-7">
                  <button onClick={() => navigateTo('home')} className="text-left text-[1.25rem] [font-variant-caps:all-small-caps] tracking-[0.04em] text-[var(--foreground)]">{text.navHome}</button>
                  {NAV_ITEMS.map((item) => (
                    <button key={item.key} onClick={() => navigateTo(item.key)} className="text-left text-[1.25rem] [font-variant-caps:all-small-caps] tracking-[0.04em] text-[var(--foreground)]">{item.label[language]}</button>
                  ))}
                </div>
              </div>
            </div>
          ...
          <footer className="border-t border-[var(--line)] py-8 mt-auto">
            <div className="container-shell">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-[0.78rem] uppercase tracking-[0.26em] text-[var(--foreground-faint)]">{text.footerLeft}</p>
                <div className="flex items-center gap-5 text-sm">
                  <a href="#" className="text-[var(--foreground-faint)] transition hover:text-[var(--foreground)]">{text.footerCredits}</a>
                  <a href="#" className="text-[var(--foreground-faint)] transition hover:text-[var(--foreground)]">{text.footerPolicy}</a>
                </div>
              </div>
            </div>
          </footer>
